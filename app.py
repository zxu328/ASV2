...
YOUR_GEMINI_API_KEY = "XXX"
# ----------------------------------------------------------------------

MODEL_NAME = "gemini-3-pro-preview"

# Detailed prompt asking for detection and reasoning, explicitly requesting JSON output
PROMPT_TEMPLATE = """
...
# =========================================================
# 2. INTEGRATED DAMAGE ASSESSMENT FUNCTION (WITH API KEY CHECK)
# =========================================================

def assess_car_damage_json(image_file_path: str):
    """
    Performs multimodal analysis on a car image and returns the result as a raw JSON string.
    """
    
    if not YOUR_GEMINI_API_KEY or "AIza" not in YOUR_GEMINI_API_KEY:
        return {"error": "API Key is missing or invalid. Please check the hardcoded value."}
    
    try:
        # Use your API key directly for initialization
    ...
    try:
        ...
        return {"error": f"Image file not found at '{image_file_path}'."}
    
    # 2. Configure the reasoning request
    config = types.GenerateContentConfig(
        response_mime_type="application/json", # Ensures a JSON response
        temperature=0.2, # Lower temperature for stable structure
        max_output_tokens=4096
    )

    # 3. Call the Gemini 3 API
    try:
        response = client.models.generate_content(
            model=MODEL_NAME,
            contents=[image, PROMPT_TEMPLATE],
            config=config
        )
    except Exception as e:
        # Catches network, rate limit, or other API-level errors
        return {"error": f"An API error occurred: {e}"}

    # 4. Return the raw JSON string, cleaned of potential markdown fences
    raw_json_string = response.text.strip()
    
    # Clean up potential markdown fences the model might include
...
# =========================================================
# 3. MODIFIED User Dashboard (Robust JSON Parsing Implemented)
# =========================================================
def show_dashboard():
    ...
    if "description_json" not in st.session_state:
        st.session_state["description_json"] = None
    ...
    if uploaded_files:
        ...
        if files_saved_count > 0:
            ...
            st.session_state["description_json"] = None
            st.session_state["pdf_data"] = None
            # Note: Rerun is not strictly needed here and was removed to show status/button immediately

    # =========================================================
    # Generate Description Section (Calls Gemini API)
    # =========================================================
    ...
    if can_start_analysis:
        ...
        # Check if we should run the analysis (button clicked OR we have files but no analysis yet)
        if st.button("Generate Description", key="gen_desc_btn") or st.session_state["description_json"] is not None:
            
            # Only run API call if not already run
            ...
            # Check if the result is an error dict from the API checker function
            if isinstance(json_string, dict) and 'error' in json_string:
                st.error(f"API Setup Error: {json_string['error']}")
                is_json_valid = False
            else:
                try:
                    # Attempt to safely parse the string into a Python dictionary/list
                    repair_data = json.loads(json_string)
                    
                    # If successful, display the dictionary as pretty JSON
                    st.json(repair_data)
                    st.success("Analysis complete! Structured JSON output received.")
                    is_json_valid = True
    
                except json.JSONDecodeError as e:
                    # This handles API stopping mid-response or invalid JSON format
                    ...
                    st.code(json_string) # Show the raw output to help diagnose the corruption
                    st.session_state["description_json"] = None # Force a re-run on next button click
                    is_json_valid = False
                except Exception:
        ...
        # =========================================================
        # Check for valid JSON after the try/except block
        is_json_valid = False
    ...
    if user_images:
        ...
        for i, img_name in enumerate(user_images):
            with cols[i % 3]:
                st.image(os.path.join(user_folder, img_name), caption=img_name, use_column_width="always")
